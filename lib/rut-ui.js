...
module.exports = function (setup, done) {
  ...
  function dontLogAuthUser(req, res, next) {
    ...
    debug('dontLogAuthUser has session "%s", %', hasSession, destination);
    if (hasSession) {
  ...
  function ensureLoggedIn(req, res, next) {
    var hasSession = req.session && req.session.passport && req.session.passport.user;
    debug('ensureLoggedIn has no session %s', !hasSession);
    if (!hasSession) {
  ...
  app.get('/client/register', [
    function renderClientRegistration(req, res) { res.render('clientRegistration'); }
  ]);
  ...
  //   dontLogAuthUser,
  //   function renderAuthorization(req, res) {
  //     res.render('login', {
  //       noUserHeader: true,
