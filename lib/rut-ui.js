...
var passGenerator = require('./pass-generator');
...
module.exports = function (setup, setupDone) {
  ...
  debug('UI styles path: %s', rutStylesPath);
  ...
  debug('admin UI styles path: %s', rutStylesPath);
  ...
  function findOrCreateForThirdParty(providerName) {
    return function findOrCreateUser(req, accessToken, refreshToken, profile, cb) {
      ...
      User.findOneByProvider(providerName, profile.username, function(err, existingUser) {
        ...
        if (existingUser) {
          if (req.isAuthenticated()) {
            ...
            req.flash('error', 'Cannot associate '+providerName+' account owned by an other user.');
        ...
        // add the provider account to logged in user
          ...
          req.flash('info', 'Your '+providerName+' account has been connected.');
          req.user.accounts.push(account);
          return req.user.save(cb);
        }
        ...
        req.flash('info', 'You successfully register with your '+providerName+' account.');
  ...
  app.get('/client/register',
    ensureLoggedIn,
    function renderClientRegistration(req, res) {
