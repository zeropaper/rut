...
(function(){
  ...
  var options = window.rutOptions || {};
  var client = RUT.client('${setup.baseUrl}/${req.rutService.name}/${req.rutService.version}.json', {
    success: function() {
      factory(client);
  ...
  /*
  RUT.client('${setup.baseUrl}/${req.rutService.name}/${req.rutService.version}.json', options)
    .then(client => {
      factory(client);
      (window.rutInit || noop)(client);
    })
