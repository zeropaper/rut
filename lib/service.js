...
service.discover = function (setup, done) {
  ...
  glob(setup.serviceYamlPattern, {cwd: setup.apiDir}, function (err, files) {
    ...
    async.each(files, function (filepath, cb) {
      ...
      Object.keys(swaggerDoc.securityDefinitions || {}).forEach(function (name) {
        var definition = swaggerDoc.securityDefinitions[name];
        if (definition.type !== 'oauth2' || definition.flow !== 'implicit') { return; }
        definition.authorizationUrl = setup.protocol + '://' + setup.host +
                                      '/authorization?client_id=' + name + version +
                                      '&response_type=' + (definition.type === 'implicit' ? 'token' : 'code') +
                                      // #facepalm.. i need something better...
                                      '&redirect_uri=' + encodeURIComponent((
                                        setup.env === 'production' ?
                                        ('https://zeropaper.github.com/' + name) :
                                        ('http://' + definition.ip + ':9091/' + name + '-' + version)
                                      ) +
                                      name);
      });
