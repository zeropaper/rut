...
module.exports = function setupMDContent(app, dataPath) {
  ...
  app.get('*', function(req, res, next) {
    ...
    console.info('markdown check for file', absPath);
    ...
    fs.stat(absPath, function(err, stats) {
      if (stats && stats.isDirectory()) {
        ...
        console.info('markdown check for file with index.md', absPath);
        return fs.stat(absPath, finalCheck);
      }

      absPath += '.md';

      console.info('markdown check for file with .md', absPath);
      return fs.stat(absPath, finalCheck);
    });
  });
};