...
const ensureAuthenticated = require('./ensure-authenticated'),
      ...
      debug               = require('debug')('rut:generic'),
      path                = require('path'),
      fs                  = require('fs');
...
module.exports = function setupModelGenerics(app, db) {
  ...
  modelNames.forEach(function(modelName) {
    ...
    function updateDoc(req, res, next) {
      var id = res.locals[docName + 'Param'].id;
      debug(`update ${modelName}: ${id}`, req.body);
      Model.findByIdAndUpdate(id, req.body, function(err) {
        if (err) return next(err);
        req.flash('success', modelName + ' has been updated');
        res.redirect('/' + modelRoute + '/' + id);
      });
    }
